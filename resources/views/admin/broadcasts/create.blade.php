@section('title', 'Buat Broadcast Baru - Admin')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Buat Broadcast Baru</h1>
        <p class="mt-2 text-gray-600">Kirim informasi penting kepada pengguna melalui tulisan berjalan</p>
    </div>
    ...
    <div class="bg-white shadow-sm rounded-lg">
        ...
        <form method="POST" action="{{ route('admin.broadcasts.store') }}" class="p-6 space-y-6">
            @csrf
            ...
            <!-- Type -->
            <div>
                <label for="type" class="block text-sm font-medium text-gray-700">Tipe Broadcast</label>
                <select name="type" id="type"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500"
                        required>
                    <option value="">Pilih Tipe</option>
                    <option value="disease_warning" {{ old('type') === 'disease_warning' ? 'selected' : '' }}>Peringatan Penyakit</option>
                    <option value="promo_feed_vaccine" {{ old('type') === 'promo_feed_vaccine' ? 'selected' : '' }}>Promo Pakan/Vaksin</option>
                    <option value="system_update" {{ old('type') === 'system_update' ? 'selected' : '' }}>Pengumuman Update Sistem</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                @error('priority')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500">Prioritas yang lebih tinggi akan ditampilkan terlebih dahulu (0 = prioritas terendah).</p>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i> Simpan Broadcast
                </button>
            </div>
        </form>
    </div>
